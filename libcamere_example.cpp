...
void LibCamera::captureFrames(int duration) {
    ...
    while (std::chrono::steady_clock::now() - start_time < std::chrono::seconds(duration)) {
        ...
        // Instead of checking for Pending, wait for the request to be completed
        request->waitForCompletion();  // This assumes a valid method for waiting

        // Process the frame
        const auto &buffers = request->buffers();
        for (const auto &[stream, buffer] : buffers) {
            if (buffer) {
                // Get the first plane from the buffer
                const FrameBuffer::Plane &plane = buffer->planes()[0];
                
                // Create a Mat using the plane's memory and size
                cv::Mat img(cv::Size(1920, 1080), CV_8UC3, plane.data(), cv::Mat::AUTO_STEP);
...
//g++ -o libcamera_example libcamera_example.cpp `pkg-config --cflags --libs libcamera opencv4`
